...
// Run once
void PixelArtApp::Setup() {
    MLOGI("PixelArt", "PixelArt Started");
}

// Run in a loop after Setup()
void PixelArtApp::Loop() {
...
void PixelArtApp::KeyEventHandler(uint16_t keyID, KeyInfo * keyInfo) {
    ...
    {
        MLOGD("PixelArt", "Key %d %d %d", xy.x, xy.y, keyInfo -> state); // Print the key event to the debug log
...
void PixelArtApp::MidiEventHandler(MidiPacket midiPacket) {
...
void PixelArtApp::UIMenu() {
    ...
    colorSelector.SetColorFunc([ & ]() -> Color {
        return color;
    }); // Use the color variable as the color of this UI element
    ...
    brightnessBtn.OnPress([ & ]() -> void {
        MatrixOS::LED::NextBrightness();
    }); // Function to call when the button is pressed
    brightnessBtn.OnHold([ & ]() -> void {
        BrightnessControl().Start();
    }); // Function to call when the button is hold down
    ...
    MLOGI("PixelArt", "Exited UI Menu");
}

void PixelArtApp::End() {
    MLOGI("PixelArt", "Example Exited");
}
